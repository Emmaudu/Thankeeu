...
const handleWallUpload = (req, res, next) => {
  upload.single('media')(req, res, (err) => {
    ...
    if (err.code === 'LIMIT_FILE_SIZE') {
      return res.status(400).json({ error: 'File too large — maximum size is 9MB. Please use a smaller photo or video.' });
    }
    ...
    if (err.message?.includes('File size too large') || err.message?.includes('Maximum is')) {
      return res.status(400).json({ error: 'File too large — please use a photo or video under 9MB.' });
    }
