    ...
    {STEPS.map((s, i) => (
      <div key={s} className="flex items-center flex-1 last:flex-none">
        <div className="flex items-center gap-2">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${
              i < current
                ...
                : i === current
                  ? "bg-primary-400 text-white ring-4 ring-primary-100"
          ...
          <span
            className={`text-sm font-medium hidden sm:block ${i <= current ? "text-gray-900" : "text-gray-400"}`}
          >
            {s}
          </span>
        </div>
        {i < STEPS.length - 1 && (
          <div
            className={`flex-1 h-0.5 mx-3 transition-all ${i < current ? "bg-primary-400" : "bg-gray-200"}`}
          />
        )}
      </div>
    ))}
...
const CreateCard = () => {
  useSEO({
    title: "Create a Card",
    description: "Create a new group card.",
    noIndex: true,
  });
  ...
  const handleOccasionSelect = (occ) => {
    set("occasion", occ.id);
    if (!form.title)
      set("title", `${user.full_name?.split(" ")[0]}'s ${occ.label} Card`);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const emails = inviteEmails
        ...
        .map((e) => e.trim())
        .filter(Boolean);
      await cardsAPI.activate(slug, { inviteEmails: emails });
      toast.success("Card created & activated! 🎉");
      navigate(`/dashboard`);
    } catch (err) {
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        ...
        {step === 0 && (
          <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8 animate-fade-in">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Pick the type of card you're creating
            </p>
        ...
        {step === 1 && (
          <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8 animate-fade-in">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Choose from our beautiful templates
            </p>
            ...
            {selectedDesign && (
              <div
                ...
                style={{ background: selectedDesign.bg }}
              >
                ...
                <div>
                  ...
                  <p
                    ...
                    style={{ color: selectedDesign.accent }}
                  >
                    This will be your card's background
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8 animate-fade-in">
            ...
            <div className="space-y-4 mb-6">
              <div>
                ...
                <input
                  className="input"
                  placeholder="e.g. Amaka's Birthday Card 🎂"
                  value={form.title}
                ...
                {[
                  {
                    ...
                    desc: "Contributors can mark messages visible only to recipient",
                  },
                  {
                    key: "send_reminders",
                    label: "Auto-send reminders",
                    desc: "Nudge people who haven't signed 2 days before deadline",
                  },
                  {
                    key: "hide_amounts",
                    label: "Hide gift amounts",
                    desc: "Contributors won't see how much others gave",
                  },
                ].map(({ key, label, desc }) => (
                  <div
                    key={key}
                  ...
                  >
                    ...
                    >
                      <span
                        className={`absolute w-5 h-5 bg-white rounded-full shadow transition-transform ${form[key] ? "translate-x-5" : "translate-x-0.5"}`}
                      />
                    </button>
                  </div>
                ))}
        ...
        {step === 3 && (
          <div className="bg-white rounded-3xl border border-gray-100 p-6 sm:p-8 animate-fade-in">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Add a gift collection and invite people to sign
            </p>
              ...
              {[
                {
                  id: true,
                  icon: "🐷",
                  title: "Enable gift pot",
                  desc: "Everyone chips in, recipient redeems",
                },
                {
                  id: false,
                  icon: "✉️",
                  title: "Card only",
                  desc: "Messages only, no gift collection",
                },
              ].map((o) => (
                <button
                  key={String(o.id)}
                  onClick={() => set("is_gift_enabled", o.id)}
                  className={`rounded-2xl p-4 text-left border-2 transition-all ${
                    form.is_gift_enabled === o.id
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {[500, 1000, 2500, 5000, 10000].map((amt) => (
                    <button
                      key={amt}
                      onClick={() => set("suggested_amount", amt)}
            ...
            <div className="mb-5">
              ...
              <textarea
                ...
                placeholder="kemi@email.com, emeka@email.com&#10;tunde@email.com"
                value={inviteEmails}
                onChange={(e) => setInviteEmails(e.target.value)}
              />
              <p className="text-xs text-gray-400 mt-1">
                You can also share a link after creating the card
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-gray-50 border border-gray-100 divide-y divide-gray-100 mb-6">
              {[
                ...
                [
                  "Design",
                  DESIGNS.find((d) => d.id === form.design_theme)?.name ||
                    form.design_theme,
                ],
                ...
                [
                  "Gift enabled",
                  form.is_gift_enabled
                    ? `Yes — ₦${form.suggested_amount.toLocaleString()} suggested`
                    : "No",
                ],
                ["Card fee", "₦1,500 one-time"],
              ].map(([k, v]) => (
                <div
                  key={k}
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(2)} className="btn-secondary px-4">
                ← Back
              </button>
              <button
              ...
              >
                {loading ? (
                  ...
                  "🔒 Pay ₦1,500 & Create Card"
                )}
              </button>
            </div>
            <p className="text-xs text-center text-gray-400 mt-3">
              Secured by Paystack · Card link will be ready immediately
            </p>
          </div>
        )}
      </div>
