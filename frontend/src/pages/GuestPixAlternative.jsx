...
const COMPETITOR = 'GuestPix';
const SLUG = '/guestpix-alternative';
...
export default function GuestPixAlternative() {
  useSEO({
    title: `The Best ${COMPETITOR} Alternative for Wedding Guest Photo Sharing | Thankeeu`,
    description: 'Looking for a GuestPix alternative? Thankeeu collects wedding photos, videos, voice notes, GIFs, wishes and cash gifts in one place — no app needed. Live gallery, Memory Movie, and permanent storage included.',
    keywords: 'guestpix alternative, better than guestpix, guestpix vs thankeeu, wedding guest photo app, wedding photo sharing QR code, collect wedding photos guests',
    canonical: SLUG,
    locale: 'en',
    jsonLd,
  });
      ...
      <section className="py-14 px-4 bg-white" id="why-alternative">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {[
              { icon: 'MessageSquare', title: 'They want more than a photo collection',
                body: 'GuestPix collects guest photos efficiently. But couples quickly realise they want the whole story — the voice of the grandmother who flew in, the written blessing from the best man, the GIF from the college friend. A photo is one layer of a wedding memory.' },
              { icon: 'Gift', title: 'They want a cash gift option without awkwardness',
                body: 'Asking guests to contribute to a honeymoon fund separately creates friction. Thankeeu embeds a gift pot directly into the same experience — guests contribute cash in their own currency alongside uploading photos.' },
              { icon: 'Film', title: 'They want a cinematic keepsake, not just files',
                body: 'A folder of 400 photos is hard to revisit. Couples want a Memory Movie — something cinematic they can watch together on every anniversary. Thankeeu auto-generates this from every contribution without any editing.' },
              { icon: 'Globe', title: 'They want it to work for international guests',
                body: 'Diaspora family in the UK, guests in the US, relatives in Nigeria — all need the same seamless experience. A WhatsApp link, a QR code, and a browser is all they need. No app. No account. No barriers.' },
              { icon: 'Mic', title: 'They want voice blessings from elders',
                body: 'A voice note from a grandparent is irreplaceable. The ability to hear someone\'s voice years later — laughing, praying, blessing — is something no photo can replicate. Thankeeu captures these moments permanently.' },
              { icon: 'Repeat', title: 'They want memories that outlive the day',
                body: 'The best wedding platforms grow with the relationship. Couples return each anniversary to add new photos and messages. Thankeeu is designed as a living memory archive, not a one-time event tool.' },
            ].map(({ icon, title, body }) => (
