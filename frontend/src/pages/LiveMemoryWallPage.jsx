...
<tbody className="divide-y divide-purple-50"> {[ ['QR code for guest photo upload', CHECK, CHECK], ['No app or account needed', CHECK, CHECK], ['Heartfelt personal messages', CROSS, CHECK], ['Voice notes & audio messages', CROSS, CHECK], ['Pooled gift collection', CROSS, CHECK], ['Auto Memory Movie™ from photos', CROSS, CHECK], ['Live display on venue screens', 'Some plans', 'Included'], ['Photo + card in one delivery', CROSS, CHECK], ['Permanent storage, no expiry', 'Paid add-on', 'Included'], ].map(([f, a, b]) => ( <tr key={f} className="hover:bg-purple-50/40 transition-colors">
...
<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5"> {[ { icon: 'QrCode', title: 'QR code — scan and upload instantly', body: 'Print on table cards, place cards, or welcome signs. Display on a projector. Guests scan and are uploading within seconds.' }, { icon: 'Smartphone', title: 'No app. No account. No friction.', body: 'The biggest reason guests don\'t share photos is friction. Thankeeu removes every barrier — no download, no registration, no password. Just tap and upload.' }, { icon: 'Radio', title: 'Live slideshow at the venue', body: 'Open the Memory Wall on any TV, projector, or big screen. Guest photos appear in real time as they upload — a live show your whole celebration watches together.' }, { icon: 'Sparkles', title: 'Full quality, no compression', body: 'Every photo and video uploads at original quality. No chat app compression, no pixelated memories. The same shot your guest took is the one you download.' }, { icon: 'Film', title: 'Auto Memory Movie™ included', body: 'After the event, Thankeeu assembles every photo, video and message into a cinematic Memory Movie™ — a film of the whole celebration made by everyone who was there.' }, { icon: 'Gift', title: 'Gift collection built in', body: 'Guests contribute to the gift alongside their photos. One link collects photos AND gifts — no separate payment link, no group chat coordination.' }, ].map(({ emoji, title, body }) => ( <div key={title} className="rounded-2xl p-5 bg-white border border-purple-100 shadow-sm">
