...
const OCCASIONS = {
  ...
  anniversary: {
    ...
    example: {
      ...
      messages: [
        { name: 'Chioma F.', msg: 'Ten years and you two still look at each other the same way you did at the wedding. That is rare. That is love. ❤️', gift: 20000 },
          ...
          <div className="rounded-3xl overflow-hidden shadow-2xl" style={{ background: data.bg, border: `2px solid ${data.accent}30` }}>
            <div className="text-center px-6 py-10" style={{ background: `linear-gradient(135deg,${data.accent}15,${data.accent}05)` }}>
              ...
              <div className="flex flex-wrap justify-center gap-3 mt-4">
                <span className="bg-white/80 rounded-full px-4 py-1.5 text-sm font-bold text-warm-700">
                  {example.messages.length} people signed
                </span>
                <span className="rounded-full px-4 py-1.5 text-sm font-bold text-white" style={{ background: data.accent }}>
              ...
              {example.messages.map((msg, i) => (
                <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-white">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {msg.gift > 0 && (
                      <span className="ml-auto text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full flex-shrink-0">
                        +{giftFmt(msg.gift)}
                      </span>
                    )}
                  </div>
                    ...
                    <div className="flex items-center gap-3 bg-purple-50 rounded-xl px-4 py-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          <div className="h-1.5 w-24 bg-primary-200 rounded-full overflow-hidden">
