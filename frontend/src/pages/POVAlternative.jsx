...
const COMPETITOR = 'POV';
const SLUG = '/pov-alternative';
...
export default function POVAlternative() {
  useSEO({
    title: `The Best ${COMPETITOR} Alternative for Wedding Guest Photo Sharing | Thankeeu`,
    description: 'Looking for a POV wedding app alternative? Thankeeu collects photos, videos, voice notes, GIFs, wishes and cash gifts in one complete wedding memory platform. No app for guests. Memory Movie auto-generated.',
    keywords: 'POV app alternative, better than POV wedding, POV vs thankeeu, wedding guest photo upload, wedding memory platform, QR wedding photos alternative',
    canonical: SLUG,
    locale: 'en',
    jsonLd,
  });
      ...
      <section className="py-14 px-4 bg-white" id="why-alternative">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {[
              { icon: 'MessageSquare', title: 'They want written messages alongside photos',
                body: 'The POV experience captures multiple camera angles. But couples also want the words — the written blessing from the father of the bride, the heartfelt message from the colleague who flew in from Lagos. Photos and words together tell the complete story.' },
              { icon: 'Gift', title: 'They want cash gifting without a separate platform',
                body: 'Coordinating a cash gift via a separate payment link creates friction and often results in fewer contributions. Thankeeu\'s native gift pot is embedded in the same experience — guests contribute money and photos from the same QR code.' },
              { icon: 'Mic', title: 'They want voice notes, not just images',
                body: 'A voice note from an elderly parent or a close friend carries emotional weight that an image cannot. Thankeeu captures these recordings permanently — in the speaker\'s own voice, in their own language, forever.' },
              { icon: 'Film', title: 'They want everything assembled into a movie',
                body: 'Thankeeu automatically turns every contribution — photo, video, message, voice note — into a cinematic Memory Movie™. The couple receives a professionally assembled film without any editing work.' },
              { icon: 'Globe', title: 'They want global guest participation',
                body: 'Diaspora guests, international friends, and relatives who could not travel all want to contribute. A link shared on WhatsApp lets anyone from anywhere upload, write, and contribute a gift — no barriers.' },
              { icon: 'Repeat', title: 'They want reuse for future occasions',
                body: 'The same platform used for the wedding can power the baby shower, the first birthday, every anniversary. Thankeeu is a family memory platform that grows across the arc of a life together.' },
            ].map(({ icon, title, body }) => (
