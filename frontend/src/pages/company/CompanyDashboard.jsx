  ...
  return (
    <CompanyLayout title={`Welcome back, ${company?.contact_person?.split(' ')[0]}! 👋`} subtitle="Here's your team celebration overview">
      ...
      {data?.today_celebrants?.length > 0 && (
        <div className="bg-gradient-to-r from-pink-50 to-primary-50 border border-pink-200 rounded-3xl p-5 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-3xl">🎂</span>
            <div>
              <p className="font-semibold text-warm-900">Birthday today!</p>
              <p className="text-sm text-warm-500">Birthday cards have been sent automatically</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-3">
            {data.today_celebrants.map(m => (
              <div key={m.id} className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 shadow-sm border border-pink-100">
                ...
                <div>
                  <p className="text-sm font-medium text-warm-900">{m.first_name} {m.last_name}</p>
                  <p className="text-xs text-warm-400">{m.department}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[...Array(4)].map((_, i) => <div key={i} className="bg-white rounded-3xl h-28 animate-pulse border border-purple-100" />)}
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-3xl border border-purple-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <h3 className="font-semibold text-warm-900">Upcoming birthdays</h3>
            <Link to="/company/teams" className="text-xs text-primary-400 hover:text-primary-600">View all →</Link>
          </div>
          <div className="divide-y divide-gray-50">
            {loading ? (
              [...Array(5)].map((_, i) => <div key={i} className="h-14 animate-pulse bg-warm-100 m-3 rounded-xl" />)
            ) : data?.upcoming_celebrants?.length === 0 ? (
              <div className="px-5 py-8 text-center text-sm text-warm-400">No upcoming birthdays in the next 30 days</div>
            ) : (
        ...
        <div className="bg-white rounded-3xl border border-purple-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <h3 className="font-semibold text-warm-900">Recent card automations</h3>
            <span className="text-xs text-warm-400">{new Date().getFullYear()}</span>
          </div>
          <div className="divide-y divide-gray-50">
            {loading ? (
              [...Array(5)].map((_, i) => <div key={i} className="h-14 animate-pulse bg-warm-100 m-3 rounded-xl" />)
              ...
              <div className="px-5 py-8 text-center">
                ...
                <p className="text-sm text-warm-400">No automations yet</p>
                <p className="text-xs text-warm-400 mt-1">Cards will appear here once birthdays are processed</p>
              </div>
            ) : (
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { icon: '⚙️', label: 'Account settings', to: '/company/settings', color: 'bg-warm-100 hover:bg-purple-50 text-warm-700' },
          { icon: '💬', label: 'Contact support', to: '/company/support', color: 'bg-green-50 hover:bg-green-100 text-green-700' },
        ].map(q => (
          <Link key={q.label} to={q.to} className={`${q.color} rounded-3xl p-4 text-center transition-all`}>
