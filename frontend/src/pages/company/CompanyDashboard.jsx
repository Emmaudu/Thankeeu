...
const CompanyDashboard = () => {
  useSEO({ title: 'HR Dashboard â€” Thankeeu for Teams', noIndex: true });
    ...
    if (params.get('sub') === 'success') {
      ...
      if (ref) {
        subscriptionAPI.verify(ref)
          .then(() => { toast.success('ðŸŽ‰ Subscription activated!'); subscriptionAPI.get().then(r => setSub(r.data)).catch(() => {}); })
          .catch(() => { subscriptionAPI.get().then(r => setSub(r.data)).catch(() => {}); toast('Payment received. Activating...', { icon: 'â³' }); });
      }
    }
  }, []);

  const isSubscribed = sub?.status === 'active';
  // expires_at is null for admin-set (free/multiplier) and pilot plans â€” they never expire.
  ...
  const neverExpires = !sub?.expires_at; // free/admin plan â€” no expiry date
  ...
  const approveM = async (id) => {
    try {
      ...
      toast.success('Member approved âœ“');
  ...
  const handleApproveScope = async (slug) => {
    ...
    try {
      ...
      toast.success('âœ… Company-wide notifications sent to all departments!');
    } catch (err) {
  ...
  return (
    <CompanyLayout title={`Welcome, ${company?.contact_person?.split(' ')[0] || 'HR'} ðŸ‘‹`} subtitle="Here's your team overview for today">
            ...
            <div key={card.id} className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="flex gap-2 flex-wrap flex-shrink-0">
                <button
                  ...
                  {approvingScope === card.slug ? 'â€¦' : 'âœ… Approve & Notify all'}
                </button>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        <Link to="/create-card" className="btn-primary text-xs py-2.5 px-4">ðŸ’Œ Create Card</Link>
        <Link to={companyPath('/occasions')} className="btn-primary text-xs py-2.5 px-4">âœ¨ Occasions</Link>
        <Link to={companyPath('/hris')} className="btn-secondary text-xs py-2.5 px-4">ðŸ“¥ HRIS &amp; Import</Link>
        <Link to={companyPath('/members')} className="btn-secondary text-xs py-2.5 px-4">ðŸ‘¥ Team members{pending.length > 0 && ` (${pending.length} pending)`}</Link>
      </div>
        ...
        <div className="mb-5 p-4 rounded-2xl" style={{ background: 'rgba(124,110,255,0.06)', border: '1px solid rgba(124,110,255,0.2)' }}>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold" style={{ color: '#1A1730' }}>
              ðŸ”” {pending.length} team member{pending.length > 1 ? 's' : ''} waiting approval
            </p>
            <Link to={companyPath('/members')} className="text-xs font-medium" style={{ color: '#7C6EFF' }}>View all â†’</Link>
          </div>
            ...
            {pending.slice(0, 3).map(m => (
              <div key={m.id} className="flex items-center gap-3 p-3 rounded-xl"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs" style={{ color: '#7A7898' }}>{m.department} Â· {m.email}</p>
                </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 mb-6">
        {loading
          ...
          : [
              statCard('ðŸ‘¥', 'Total employees', stats.total_members || 0, 'all departments'),
              statCard('ðŸ¢', 'Teams / Depts', stats.teams || (stats.departments || []).length || 0, 'active departments'),
              statCard('ðŸŽ‰', 'Upcoming occasions', upcoming.length, 'next 30 days'),
              statCard('ðŸ’Œ', 'Active cards', stats.active_cards || 0, 'collecting now'),
              statCard('ðŸŽ', 'Gifts collected', `â‚¦${((stats.total_collected||0)).toLocaleString('en-NG')}`, 'all time', true),
            ]
        }
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="rounded-2xl border-2 overflow-hidden" style={{ background: '#fff', borderColor: '#EDE9FF' }}>
          <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid #EDE9FF' }}>
            <h3 style={{ fontFamily: 'Space Grotesk,sans-serif', fontWeight: 600, fontSize: '0.95rem', color: '#1A1730' }}>
              ðŸŽ‰ Upcoming occasions
            </h3>
            <Link to={companyPath('/teams')} className="text-xs font-medium" style={{ color: '#7C6EFF' }}>View all â†’</Link>
          </div>
          {loading ? (
                ...
                return (
                  <div key={i} className="flex items-center gap-3 px-5 py-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0"
                      ...
                      {occ.occasion_type === 'birthday' ? 'ðŸŽ‚' : occ.occasion_type === 'leaving' ? 'ðŸ‘‹' : occ.occasion_type === 'promotion' ? 'ðŸŒŸ' : occ.occasion_type === 'fathers_day' ? 'ðŸ‘”' : occ.occasion_type === 'womens_day' ? 'ðŸ‘©' : 'ðŸŽ‰'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold truncate" style={{ color: '#1A1730' }}>{occ.name || `${occ.first_name||''} ${occ.last_name||''}`.trim()}</p>
                      <p className="text-xs" style={{ color: '#7A7898' }}>{occ.label || occ.occasion_type?.replace(/_/g,' ')} Â· {occ.department}</p>
                    </div>
                    <span className="text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0"
                      style={{ background: urgent ? 'rgba(239,68,68,0.1)' : 'rgba(124,110,255,0.1)', color: urgent ? '#dc2626' : '#5B4BDF' }}>
                      {days === 0 ? 'Today! ðŸŽ‰' : `${days}d`}
                    </span>
                  </div>
                );
              })}
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl p-5 border-2" style={{ background: '#fff', borderColor: '#EDE9FF' }}>
            <p className="text-xs font-semibold mb-2" style={{ color: '#7A7898' }}>ðŸ”‘ Your company invite code</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { to: companyPath('/teams'), icon: 'ðŸŽ‰', label: 'Manage occasions', sub: 'Add birthday dates' },
              { to: companyPath('/members'), icon: 'ðŸ‘¥', label: 'Team members', sub: 'Import & approve' },
              { to: companyPath('/hris'), icon: 'ðŸ”—', label: 'HRIS sync', sub: 'Connect SeamlessHR' },
              { to: companyPath('/subscription'), icon: 'ðŸ’³', label: 'Subscription', sub: isSubscribed ? (neverExpires ? 'Active Â· No expiry' : `${daysLeft}d remaining`) : 'Not active' },
