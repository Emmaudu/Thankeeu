...
const QRButton = ({ url, label = 'QR code', className = '', children }) => {
  ...
  return (
    <>
      <button
        ...
        className={className || 'text-xs border border-purple-200 text-warm-600 hover:bg-warm-100 px-3 py-2 rounded-xl transition-colors'}
      >
        {children || 'QR'}
      </button>
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-xs rounded-2xl bg-white p-5 text-center shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <img src={qrUrl} alt={label} className="mx-auto mt-4 h-56 w-56 rounded-xl border border-purple-100 bg-white p-2" />
            <p className="mt-3 break-all text-xs text-warm-500">{url}</p>
            <button type="button" onClick={() => setOpen(false)} className="btn-secondary mt-4 w-full py-2 text-sm">Close</button>
          </div>
        </div>
      )}
    </>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            {card.is_gift_enabled && onToggleHideAmounts && (
              ...
              >
                {toggling ? '...' : card.hide_amounts ? 'Total hidden' : 'Total visible'}
              </button>
            )}
          </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {(card.status === 'draft' || card.status === 'active') && (
          <Link to={`/create-card?edit=${card.slug}`}
            ...
            {card.status === 'active' ? 'Edit card' : 'Edit draft'}
          </Link>
        )}
        {card.status === 'active' && (
