  ...
  return (
    <CompanyLayout title="Support" subtitle="Get help from the Thankeeu team">
      <div className="max-w-2xl">
        ...
        {/* New ticket form */}
        <div className="bg-white rounded-3xl border border-purple-100 p-6 mb-8">
          <h3 className="font-semibold text-warm-900 mb-1">Send a message to Thankeeu Support</h3>
          <p className="text-sm text-warm-500 mb-5">Describe your issue and we'll get back to you as soon as possible.</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-warm-700 mb-1.5">Subject <span className="text-red-400">*</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-warm-700 mb-1.5">Message <span className="text-red-400">*</span></label>
              <textarea
              ...
              <div className="flex justify-between mt-1">
                <p className="text-xs text-warm-400">Be as specific as possible to help us resolve this faster</p>
                <span className="text-xs text-warm-400">{form.message.length}/2000</span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <a href="mailto:support@thankeeu.com" className="bg-white border border-purple-100 rounded-3xl p-4 hover:border-primary-300 transition-colors text-center">
            ...
            <p className="text-xs text-warm-400 mt-0.5">support@thankeeu.com</p>
          </a>
          <div className="bg-white border border-purple-100 rounded-3xl p-4 text-center">
            <div className="text-2xl mb-2">⏱️</div>
            <p className="text-sm font-medium text-warm-900">Response time</p>
            <p className="text-xs text-warm-400 mt-0.5">Within 24 hours</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-purple-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
            <h3 className="font-semibold text-warm-900">Your support tickets</h3>
            <span className="text-xs text-warm-400">{tickets.length} tickets</span>
          </div>

          {loading ? (
            <div className="p-5 space-y-3">
              {[...Array(3)].map((_, i) => <div key={i} className="h-16 bg-warm-100 rounded-xl animate-pulse" />)}
            </div>
          ) : tickets.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <div className="text-4xl mb-3">🎫</div>
              <p className="text-sm text-warm-400">No support tickets yet</p>
              <p className="text-xs text-warm-400 mt-1">Use the form above to reach out</p>
            </div>
          ) : (
                ...
                <div key={t.id}>
                  <button
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-warm-400">{format(new Date(t.created_at), 'MMM d, yyyy · h:mm a')}</p>
                    </div>
                    <span className="text-warm-400 flex-shrink-0 mt-0.5">{openTicket === t.id ? '▲' : '▼'}</span>
                  </button>

                  {openTicket === t.id && (
                    <div className="px-5 pb-5 space-y-3">
                      <div className="bg-warm-100 rounded-xl p-4">
                        <p className="text-xs font-semibold text-warm-500 mb-2">Your message</p>
                      ...
                      {t.admin_reply && (
                        <div className="bg-primary-50 border border-primary-100 rounded-xl p-4">
                          ...
                          <p className="text-sm text-warm-800 leading-relaxed whitespace-pre-line">{t.admin_reply}</p>
                        </div>
                      )}
                      {!t.admin_reply && (
