...
const daysUntil = (dateStr) => {
  useSEO({
    title: "Team Occasions — Thankeeu for Teams",
    description: "Manage all employee occasions.",
    noIndex: true,
  });
...
const TeamsPage = () => {
  ...
  useEffect(() => {
    if (selected) fetchMembers();
  }, [selected, search, deptFilter]);

  const fetchMembers = async () => {
    setMembersLoading(true);
    try {
      const res = await occasionsAPI.getMembers(selected.id, {
        search,
  ...
  const handleDownload = async () => {
    try {
      ...
      a.href = url;
      a.download = `thankeeu_${selected.name}_template.xlsx`;
      a.click();
  ...
  const handleImport = async (e) => {
    ...
    if (!file.name.match(/\.(xlsx|xls)$/))
      return toast.error("Upload an Excel file (.xlsx or .xls)");
    setImporting(true);
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      await occasionsAPI.deleteMember(selected.id, id);
  ...
  const dateColName = (name) =>
    ({
      ...
      leaving: "Last Day (DD-MM-YYYY)",
      work_anniversary: "Start Date (DD-MM-YYYY)",
      retirement: "Retirement Date (DD-MM-YYYY)",
    })[name] || "Date (DD-MM-YYYY)";

  return (
    <CompanyLayout
      title="Occasion Tables"
      subtitle="Manage employee data for each celebration occasion"
    >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-amber-800 text-sm">
                Import is free. Subscribe to activate automated emails.
              </p>
              <p className="text-xs text-amber-600 mt-0.5">
                Once subscribed, the system sends cards automatically for every
                occasion.
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-gray-800 text-sm">
              Connect your HRIS to auto-populate all occasion tables
            </p>
            <p className="text-xs text-gray-500 mt-0.5">
              SeamlessHR, BambooHR, Zoho People, SAP SuccessFactors, WorkPay —
              one sync fills everything
            </p>
          </div>
        </div>
        <Link
          to="/company/hris"
          className="bg-primary-400 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-primary-600 transition-colors whitespace-nowrap flex-shrink-0"
        >
          🔗 Connect HRIS →
        </Link>
      </div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
        {loading
          ? [...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-10 w-32 bg-gray-100 rounded-xl animate-pulse flex-shrink-0"
              />
            ))
          : types.map((ot) => (
              <button
              ...
              >
                ...
                {ot.member_count > 0 && (
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${selected?.id === ot.id ? "bg-white/20 text-white" : "bg-gray-100 text-gray-500"}`}
                  >
                    {ot.member_count}
                  </span>
                )}
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {selected.default_scope === "company_wide"
                    ? "Company-wide"
                    : "Department"}{" "}
                  notifications ·{selected.notify_days_before}d notice ·
                  {members.length} members
                  {selected.gender_filter
                    ? ` · ${selected.gender_filter}s only`
                    : ""}
                </p>
              </div>
            </div>
          ...
          {!membersLoading && members.length === 0 && (
            <div className="bg-primary-50 rounded-2xl p-5 mb-5 border border-primary-100">
              <p className="text-sm font-semibold text-primary-700 mb-3">
                📋 {selected.label} template columns
              </p>
              <div className="flex flex-wrap gap-2 mb-3">
                {[
                  ...
                  dateColName(selected.name),
                  "Notes (optional)",
                ].map((c) => (
                  <span
                    key={c}
                    className="text-xs bg-white text-primary-600 px-2.5 py-1.5 rounded-lg border border-primary-200 font-medium"
                  >
                    {c}
                  </span>
                ))}
              ...
              <p className="text-xs text-primary-500">
                Unlimited employees. Data import is always free.
              </p>
            </div>
          )}
          ...
          {importResult && (
            <div className="mb-5 bg-green-50 border border-green-200 rounded-2xl p-4">
              <p className="text-sm font-semibold text-green-800 mb-1">
                ✅ {importResult.imported} imported to {selected.label}
              </p>
          ...
          {members.length > 0 && (
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              <input
                ...
                placeholder={`Search ${selected.label} members...`}
                value={search}
            ...
            {membersLoading ? (
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-sm font-medium text-gray-700 mb-1">
                  No {selected.label} data yet
                </p>
                <p className="text-xs text-gray-400 mb-5">
                  Download the template, fill it in, then upload it back here
                </p>
                <button
                  onClick={handleDownload}
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">
                            {new Date(m.occasion_date).toLocaleDateString(
                              "en-NG",
