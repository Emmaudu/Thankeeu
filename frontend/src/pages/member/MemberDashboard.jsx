      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <h3 className="font-semibold text-gray-900">Upcoming occasions</h3>
            <Link to="/member/occasions" className="text-xs text-primary-400 hover:text-primary-600">All →</Link>
          </div>
          <div className="divide-y divide-gray-50">
            {loading ? (
              [...Array(4)].map((_, i) => <div key={i} className="h-14 animate-pulse bg-gray-50 m-3 rounded-xl" />)
            ) : (data?.upcoming_occasions || []).length === 0 ? (
              <div className="px-5 py-8 text-center text-sm text-gray-400">No upcoming occasions in 30 days</div>
            ) : (
              (data?.upcoming_occasions || []).slice(0, 6).map(m => (
                <div key={m.id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{m.occasion_types?.label} · {m.department}</p>
                  </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <h3 className="font-semibold text-gray-900">Active cards to sign</h3>
          </div>
          <div className="divide-y divide-gray-50">
            {loading ? (
              [...Array(4)].map((_, i) => <div key={i} className="h-14 animate-pulse bg-gray-50 m-3 rounded-xl" />)
              ...
              <div className="px-5 py-8 text-center">
                ...
                <p className="text-sm text-gray-400">No active cards right now</p>
                {isLeader && (
      ...
      {isLeader && (
        <div className="mt-6 bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50">
            <h3 className="font-semibold text-gray-900">Your department — {member?.department}</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {loading ? (
                  <tr><td colSpan={4} className="p-4"><div className="h-8 bg-gray-100 rounded animate-pulse" /></td></tr>
                ) : (data?.dept_members || []).length === 0 ? (
                  <tr><td colSpan={4} className="px-5 py-8 text-center text-sm text-gray-400">No approved members in your department yet</td></tr>
                ) : (
