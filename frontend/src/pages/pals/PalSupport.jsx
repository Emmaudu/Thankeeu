...
export default function PalSupport() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await palAPI.createTicket(form);
  ...
  const STATUS_COLOR = s => ({ open:'bg-amber-100 text-amber-700', answered:'bg-green-100 text-green-700', closed:'bg-gray-100 text-gray-600' }[s]||'bg-gray-100 text-gray-600');

  return (
    <PalLayout title="Support" subtitle="Message Thankeeu — we typically reply within 24 hours">
